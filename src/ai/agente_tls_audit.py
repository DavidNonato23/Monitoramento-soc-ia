...
def analisar_certificado_tls(host: str, porta: int = 443) -> dict[str, Any]:
    ...
    NOTA: verify_mode=CERT_NONE é intencional aqui — o objetivo é AUDITAR
    o certificado (mesmo que inválido/expirado/autoassinado), não validar
    uma conexão seguros. A avaliação de validade acontece depois, via IA,
    olhando os dados extraídos.
    """
...
def executar_auditoria_tls(host: str, porta: int = 443) -> dict[str, Any]:
    """
    Realiza a inspeção técnica de TLS e submete os dados ao Groq para parecer de segurança.
    """
