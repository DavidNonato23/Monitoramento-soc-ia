...
@app.route("/servidor/<int:id>/backup", methods=["POST"])
@requer_autenticacao
...
def gerar_certificado_pdf(id):
    ...
    nome_arquivo = f"certificado_conformidade_{nome_seguro}_{id}.pdf"
    ...
    c.rect(0, altura - 100, largura, 100, fill=1, stroke=0)

    c.setFillColorRGB(0.12, 0.73, 0.45)
    c.setFont("Helvetica-Bold", 18)
    c.drawString(50, altura - 50, "VANGUARDSEC AI COMMAND CENTER")

    c.setFillColorRGB(1, 1, 1)
    c.setFont("Helvetica", 11)
    c.drawString(50, altura - 75, "Certificado Oficial de Conformidade, Certificados & Licenciamento (ISO 27001)")

    c.setFillColorRGB(0.1, 0.1, 0.1)
    c.setFont("Helvetica-Bold", 14)
    c.drawString(50, altura - 140, f"Host Auditado: {srv['nome']}")

    c.setFont("Helvetica", 11)
    c.drawString(50, altura - 170, f"Endereço IP: {srv['ip']}:{srv['porta']}")
    c.drawString(50, altura - 195, f"Função no Ambiente: {srv['funcao']}")
    c.drawString(50, altura - 220, f"Data da Auditoria: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    c.setStrokeColorRGB(0.7, 0.7, 0.7)
    c.line(50, altura - 240, largura - 50, altura - 240)

    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, altura - 275, "Escopo de Ativos Auditados e Conformidade:")

    c.setFont("Helvetica", 11)
    c.drawString(70, altura - 305, "• Certificados SSL/TLS / mTLS: Validados e dentro da vigência preventiva")
    c.drawString(70, altura - 335, "• Licenciamento de Sistema Operacional & Pacotes: Ativos e regulares")
    c.drawString(70, altura - 365, "• Assinaturas e Chaves de Criptografia: Conformes com ISO 27001")
    c.drawString(70, altura - 395, "• Perímetro e Firewall (UFW): Sob monitoramento autônomo (SOAR)")

    c.setFont("Helvetica-Oblique", 9)
    c.setFillColorRGB(0.4, 0.4, 0.4)
    c.drawString(50, 50, "Este documento foi gerado automaticamente pelo Agente Tier 2 do VanguardSec AI.")
...
# --- ROTAS DE INTEGRAÇÃO DE AUDITORIA E PENTEST ---

@app.route("/api/pentest-nmap", methods=["POST"])
@requer_autenticacao
def api_pentest_nmap():
    ip_alvo = request.form.get("ip") or (request.json.get("ip") if request.is_json else None)
    if not ip_alvo:
...
@app.route("/api/pentest-cve-lookup/<int:id>", methods=["POST"])
@requer_autenticacao
...
@app.route("/api/pentest-tls-audit/<int:id>", methods=["POST"])
@requer_autenticacao
def api_pentest_tls_audit(id):
    ...
    porta_target = int(request.form.get("porta", 443))
...
@app.route("/api/pentest-backup-audit/<int:id>", methods=["POST"])
@requer_autenticacao
